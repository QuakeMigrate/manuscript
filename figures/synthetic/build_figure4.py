...
fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(7.08661, 6.8), constrained_layout=True)

colours = make_colours()
station_names = ["STA7", "STA8", "STA5", "STA3", "STA6", "STA0", "STA9", "STA1", "STA2", "STA4"]
ticks = []

simulated_stream = obspy.read("./generate_synthetic_results/inputs/mSEED/2021/049/STA*.m")
...
spine_weight = 1.2
labels = ["Vertical component (P)", "P Onset", "Horizontal component (S)", "S Onset"]
for ax, panel_label, label in zip(axes.flatten(), "abcd", labels):
    ax.text(0.03, 0.96, panel_label, ha="center", va="center",
            transform=ax.transAxes, fontweight="bold")
    ax.text(0.985, 0.96, label, ha="right", va="center",
            transform=ax.transAxes)
    plt.setp(ax.spines.values(), linewidth=spine_weight)

for ax in [axes.flatten()[0], axes.flatten()[2]]:
    ax.set_yticks(ticks)
    ax.set_yticklabels(station_names, size=7, fontname="monospace", color="w",
                       fontweight="bold")
...
plt.savefig("figure4.pdf", dpi=400)
